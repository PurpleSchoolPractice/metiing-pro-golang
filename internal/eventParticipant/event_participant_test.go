...
func TestGetEventParticipants(t *testing.T) {
	...
	mock.ExpectQuery(
		`SELECT users.* FROM "users" JOIN event_participants ON 
    users.id = event_participants.user_id WHERE event_participants.event_id = $1`,
	).
		WithArgs(uint(1)).
...
func TestGetUserEvents(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT events.* FROM "events" JOIN event_participants ON 
    events.id = event_participants.event_id WHERE event_participants.user_id = $1`).
		WithArgs(uint(1)).
		WillReturnRows(sqlmock.NewRows([]string{
	...
	require.Equal(t, "description", events[0].Description)
	require.Equal(t, date, events[0].EventDate)
	require.Equal(t, uint(1), events[0].CreatorID)
	require.NoError(t, mock.ExpectationsWereMet())
}
