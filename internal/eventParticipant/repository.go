...
func (repo *EventParticipantRepository) GetEventParticipants(eventID uint) ([]types2.User, error) {
	var users []types2.User
	result := repo.DataBase.DB.
		Joins("JOIN event_participants ON event_participants.user_id = users.id").
...
func (repo *EventParticipantRepository) GetUserEvents(userID uint) ([]types2.Event, error) {
